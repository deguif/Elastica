 ...
 * @internal
 */
class FunctionScoreQueryTest extends BaseTest
{
    ...
    public function testToArray(): void
    {
        ...
        $childQuery = new MatchAllQuery();
        $query->setQuery($childQuery);
        $query->addDecayFunction(FunctionScoreQuery::DECAY_GAUSS, 'location', $this->locationOrigin, $locationScale);
        $query->addDecayFunction(FunctionScoreQuery::DECAY_GAUSS, 'price', $priceOrigin, $priceScale);
        $expected = [
            'function_score' => [
    ...
    public function testDecayWeight(): void
    {
        ...
        $query->addDecayFunction(
            FunctionScoreQuery::DECAY_GAUSS,
            'location',
            $this->locationOrigin,
            $locationScale,
            null,
            ...
            .5,
            null,
            FunctionScoreQuery::MULTI_VALUE_MODE_AVG
        );
        $query->addDecayFunction(
            FunctionScoreQuery::DECAY_GAUSS,
            'price',
            ...
            2,
            null,
            FunctionScoreQuery::MULTI_VALUE_MODE_MAX
        );
        $expected = [
            'function_score' => [
                'query' => $childQuery->toArray(),
                'functions' => [
                    [
                        'gauss' => [
                            ...
                            'multi_value_mode' => FunctionScoreQuery::MULTI_VALUE_MODE_AVG,
                        ],
                        'weight' => .5,
                    ],
                    [
                        'gauss' => [
                            ...
                            'multi_value_mode' => FunctionScoreQuery::MULTI_VALUE_MODE_MAX,
                        ],
                        'weight' => 2,
                    ],
                ],
    ...
    {
        $query = new FunctionScoreQuery();
        $query->addDecayFunction(FunctionScoreQuery::DECAY_GAUSS, 'location', $this->locationOrigin, '4mi');
        $query->addDecayFunction(FunctionScoreQuery::DECAY_GAUSS, 'price', 0, 10);
    ...
    public function testGaussMultiValue(): void
    {
        ...
        $query->addDecayFunction(
            ...
            null,
            null,
            FunctionScoreQuery::MULTI_VALUE_MODE_SUM
        );
        $response = $this->_getIndexForTest()->search($query);
        $results = $response->getResults();
    ...
    {
        $filter = new TermQuery(['price' => 4.5]);
        $query = new FunctionScoreQuery();
        $query->addWeightFunction(5.0, $filter);

        $sameFilter = new TermQuery(['price' => 4.5]);
        $sameQuery = new FunctionScoreQuery();
    ...
    public function testSetMinScore(): void
    {
        ...
        $query = (new FunctionScoreQuery())
            ->addDecayFunction(FunctionScoreQuery::DECAY_GAUSS, 'price', 0, 10)
            ->setMinScore(0.8)
        ;
    ...
    public function testFieldValueFactor(): void
    {
        ...
        $query = new FunctionScoreQuery();
        $query->addFieldValueFactorFunction('popularity', 1.2, FunctionScoreQuery::FIELD_VALUE_FACTOR_MODIFIER_SQRT, 0.1);

 ...
 * @internal
 */
class ParentIdQueryTest extends BaseTest
{
    /**
     * @group unit
     */
    ...
    {
        $query = new ParentIdQuery('join', '1');
        $expectedArray = [
            'parent_id' => [
                'type' => 'join',
                'id' => 1,
    ...
    public function testParentId(): void
    {
        ...
        $parentQuery = new ParentIdQuery('answer', 1, true);
        ...
        $parentQuery = new ParentIdQuery('answer', '2', true);
        $search = new Search($index->getClient());
        $results = $search->search($parentQuery);

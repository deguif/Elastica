 ...
 * @internal
 */
class SpanContainingQueryTest extends BaseTest
{
    /**
     * @group unit
     */
    public function testToArray(): void
    {
        ...
        $spanContainingQuery = new SpanContainingQuery($spanTermQuery3, $spanNearQuery);
    ...
    public function testSpanContaining(): void
    {
        ...
        $spanContainingQuery = new SpanContainingQuery(new SpanTermQuery([$field => 'amet']), $spanNearQuery);
        $resultSet = $index->search($spanContainingQuery);
        $this->assertEquals(1, $resultSet->count());

        $spanContainingQuery = new SpanContainingQuery(new SpanTermQuery([$field => 'not-matching']), $spanNearQuery);
        $resultSet = $index->search($spanContainingQuery);

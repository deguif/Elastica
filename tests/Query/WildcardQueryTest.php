 ...
 * @internal
 */
class WildcardQueryTest extends BaseTest
{
    /**
     * @group unit
     */
    ...
    {
        $wildcard = new WildcardQuery('name', 'aaa*');
    ...
    {
        $wildcard = new WildcardQuery('name', 'value*', 2.0);
        $wildcard->setRewrite(WildcardQuery::REWRITE_SCORING_BOOLEAN);
    ...
    public function testSearchWithAnalyzer(): void
    {
        ...
        $query = new WildcardQuery('name', 'ba*');
        $resultSet = $index->search($query);

        $this->assertEquals(3, $resultSet->count());

        $query = new WildcardQuery('name', 'baden*');
        $resultSet = $index->search($query);

        $this->assertEquals(2, $resultSet->count());

        $query = new WildcardQuery('name', 'baden b*');
        $resultSet = $index->search($query);

        $this->assertEquals(1, $resultSet->count());

        $query = new WildcardQuery('name', 'baden bas*');
        $resultSet = $index->search($query);
